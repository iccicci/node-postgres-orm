...
function createLoadFK(tbl, nam, fld) {
	return function(callback) {
		var where = {};

		where[fld] = this[nam];

		this.pgo.tables[tbl].load(where, null, callback, this.__tx, null, this.pgo);
	};
}

function createLockFK(tbl, nam, fld) {
	return function(callback) {
		var where = {};

		where[fld] = this[nam];

		this.pgo.tables[tbl].load(where, null, callback, this.__tx, true, this.pgo);
	};
}
...
function createClasses(pgo) {
	...
	for(i = 0; i < fields.length; ++i)
		if(fields[i].fkTable) {
			...
			Rcd.prototype[nam + "Lock"] = createLockFK(fields[i].fkTable, nam, fields[i].fkField);
		}
}
