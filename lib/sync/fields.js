...
function createField(pgo) {
	...
	singleStep(pgo, table, "ALTER TABLE " + table.tname + " ADD COLUMN " + field.name + " " + fieldType(field), 1004, syncField.bind(null, pgo, true), field);
}
...
function dropFields(pgo) {
	...
	pgo.client.query("SELECT attname FROM pg_attribute WHERE attrelid = $1 AND attnum > 0 AND attisdropped = false", [table.oid], function(err, res) {
		...
		finalizeSync("ALTER TABLE " + table.tname + " DROP COLUMN " + field + " CASCADE", pgo, 1014, table, field, dropFields);
	});
}
...
function syncFieldNull(pgo, resin) {
	...
	var doit = function() {
		...
		finalizeSync(query, pgo, 1019, table, field.name, syncField);
	};
...
function syncFieldDefault(pgo, resin) {
	...
	var doIt = function() {
		...
		finalizeSync(query, pgo, 1018, table, field.name, syncFieldNull.bind(null, pgo, resin));
	};
...
function alterField(pgo, resin) {
	...
	finalizeSync("ALTER TABLE " + table.tname + " ALTER COLUMN " + field.name + " TYPE " + fieldType(field) + using, pgo, 1003, table, field.name, syncFieldDefault.bind(null, pgo, resin));
}
