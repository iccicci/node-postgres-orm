...
function fieldType(field) {
  ...
  case "varchar":
    if(field.maxLen) return "varchar(" + field.maxLen + ")";
  }
...
function dropFields(pgo) {
  ...
  pgo.client.query("SELECT attname FROM pg_attribute WHERE attrelid = $1 AND attnum > 0 AND attisdropped = false", [table.oid], function(err, res) {
    if(pgo.error(err, 1022, table.name)) return;

    var field;
    var fields = pgo.tables[table.name].__fields;
    var rows = res.rows;

    for(var i in rows) if(! (rows[i].attname in fields)) field = rows[i].attname;

    if(! field) return syncConstraint(pgo);
...
function syncFieldNull(pgo, resin) {
  ...
  if(resin.attnotnull === field.notNull) return syncField(pgo);
  ...
  if(field.notNull && field.defaultValue) {
    ...
    pgo.client.query(query, null, function(err, res) {
      if(pgo.error(err, 1021, table.name, field.name)) return;
...
function syncFieldDefault(pgo, resin) {
  ...
  if(
    resin.adsrc == field.defaultValue || // eslint-disable-line eqeqeq
  ...
  if(field.type !== "timestamptz" || field.defaultValue === "CURRENT_TIMESTAMP") return doIt();

  pgo.client.query("SELECT " + field.defaultValue + "::character varying", null, function(err, res) {
    if(pgo.error(err, 1025, table.name, field.name)) return;

    if(resin.adsrc === "'" + res.rows[0].varchar + "'::timestamp with time zone") return syncFieldNull(pgo, resin);
...
function syncField(pgo, redo) {
  ...
  if(schema.fidx === table.fields.length) return dropFields(pgo);
  ...
  pgo.client.query(
    ...
    [table.oid, field.name],
    function(err, res) {
      if(pgo.error(err, 1013, table.name)) return;

      if(! res.rowCount) return createField(pgo);

      var row = res.rows[0];

      if(field.type !== row.typname) return alterField(pgo, row);

      if(field.type === "varchar" && (field.maxLen ? field.maxLen + 4 !== row.atttypmod : row.atttypmod !== -1)) return alterField(pgo, row);
