...
function createSingleStep(before, after, code, next) {
  return function(pgo) {
    var table = getTable(pgo);

    singleStep(pgo, table, before + table.tname + after, code, next);
  };
}
...
function arrayCompare(a1, a2) {
  ...
  for(var i in a1) if(a2.indexOf(a1[i]) === -1) return false;
...
function createIndex(pgo) {
  ...
  if(! table.index || schema.iidx === table.index.length) return syncSequence(pgo);
  ...
  pgo.client.query(
    ...
    [table.oid, index.type],
    function(err, res) {
      if(pgo.error(err, 1024, table.name)) return;
      ...
      for(var i in rows) {
        ...
        if(row.relname !== idx) {
          if(idx) if(arrayCompare(index.fields, fld)) return createIndex(pgo);

          idx = row.relname;
          fld = [row.attname];
        } else fld.push(row.attname);
      }

      singleStep(pgo, table, "CREATE INDEX ON " + table.tname + " USING " + index.type + " (" + index.fields.join(", ") + ")", 1007, createIndex, index.fields);
    }
  );
}
...
function syncConstraint(pgo) {
  ...
  if(schema.cidx === table.constraints.length) return createIndex(pgo);
  ...
  pgo.client.query("SELECT attname FROM pg_attribute, pg_constraint WHERE attrelid = $1 AND conrelid = $1 AND attnum = conkey[1] AND attname = $2", [table.oid, constraint.field], function(err, res) {
    if(pgo.error(err, 1023, table.name)) return;

    if(res.rowCount) return syncConstraint(pgo);
    ...
    singleStep(pgo, table, query, 1017, syncConstraint, constraint.name);
  });
}
...
function dropIndex(pgo, arr) {
  ...
  pgo.client.query(query + " ORDER BY relname", values, function(err, res) {
    if(pgo.error(err, 1006, table.name)) return;
    ...
    for(var i in rows) {
      ...
      if(row.relname !== idx) {
        if(idx) {
          ...
          for(var l in table.index) if(arrayCompare(table.index[l].fields, fld) && table.index[l].type === am) found = true;
    ...
    if(! found) {
      ...
      return pgo.client.query("DROP INDEX " + idx, null, function(err, res) {
        if(pgo.error(err, 1015, table.name)) return;
...
function dropConstraint(pgo) {
  ...
  pgo.client.query(query + wheres.join(""), values, function(err, res) {
    if(pgo.error(err, 1010, table.name)) return;
    ...
    if(! res.rowCount) {
      return pgo.client.query("SELECT conindid FROM pg_attribute, pg_constraint WHERE attrelid = $1 AND conrelid = $1 AND attnum = conkey[1]", [table.oid], function(err, res) {
        ...
        for(var i in res.rows) arr.push(res.rows[i].conindid);
    ...
    singleStep(pgo, table, "ALTER TABLE " + table.tname + " DROP CONSTRAINT " + conname, 1012, dropConstraint, conname);
  });
}

var dropTable = createSingleStep("DROP TABLE ", " CASCADE", 1016, createTable);

function syncParent(pgo) {
  var table = pgo.schema.tables[pgo.schema.tidx];

  pgo.client.query("SELECT inhparent FROM pg_inherits WHERE inhrelid = $1", [table.oid], function(err, res) {
    if(pgo.error(err, 1020, table.name)) return;

    if(! table.parent) {
      if(res.rowCount) return dropTable(pgo);
    ...
    if(! res.rowCount || res.rows[0].inhparent !== parent.oid) return dropTable(pgo);
...
function syncTable(pgo) {
  ...
  pgo.client.query("SELECT oid FROM pg_class WHERE relname = $1", [table.tname], function(err, res) {
    if(pgo.error(err, 1005, table.name)) return;

    if(! res.rowCount) return createTable(pgo);
...
var createSequence = createSingleStep("CREATE SEQUENCE ", "_id_seq", 1009, syncTable);
...
function syncSequence(pgo) {
  ...
  if(table.parent || table.primaryKey) return syncTable(pgo);
  ...
  pgo.client.query("SELECT currval('" + table.tname + "_id_seq')", null, function(err, res) {
    ...
    if(err.code === "42P01") return createSequence(pgo);
    if(err.code === "55000") return syncTable(pgo);
