...
var syncField = require("./fields")(syncConstraint);
...
function createIndex(pgo) {
	...
	pgo.client.query("SELECT relname, attname FROM pg_class, pg_index, pg_attribute, pg_am WHERE indrelid = $1 AND indexrelid = pg_class.oid AND attrelid = pg_class.oid AND relam = pg_am.oid AND amname = $2 ORDER BY relname", [
		table.oid, index.type], function(err, res) {
...
function syncConstraint(pgo) {
	...
	pgo.client.query("SELECT attname FROM pg_attribute, pg_constraint WHERE attrelid = $1 AND conrelid = $1 AND attnum = conkey[1] AND attname = $2", [
		table.oid, constraint.field], function(err, res) {
...
function createTable(pgo) {
	var table = getTable(pgo);
	var parent = table.parent ? " INHERITS (" + table.parent + "s)" : "";

	singleStep(pgo, table, "CREATE TABLE " + table.name + "s ()" + parent, 1008, syncTable);
}
...
function dropConstraint(pgo) {
	...
	pgo.client.query(query + wheres.join(""), values, function(err, res) {
		...
		if(! res.rowCount) {
			return pgo.client.query("SELECT conindid FROM pg_attribute, pg_constraint WHERE attrelid = $1 AND conrelid = $1 AND attnum = conkey[1]", [
...
function dropTable(pgo) {
	...
	singleStep(pgo, table, "DROP TABLE " + table.name + "s CASCADE", 1016, createTable);
}

function syncParent(pgo) {
	var table = pgo.schema.tables[pgo.schema.tidx];

	pgo.client.query("SELECT inhparent FROM pg_inherits WHERE inhrelid = $1", [
		table.oid], function(err, res) {
...
function syncTable(pgo) {
	...
	pgo.client.query("SELECT oid FROM pg_class WHERE relname = $1", [
		table.name + "s"], function(err, res) {
...
function createSequence(pgo) {
	...
	singleStep(pgo, table, "CREATE SEQUENCE " + table.name + "s_id_seq", 1009, syncTable);
}
