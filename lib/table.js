...
function doReload(pgo, reload, ret, done, doneOk, keys, idx, tx) {
	...
	table.load(where, null, done, (res) => {
		for(var i in res)
			ret[obj[res[i][
				table.pk]]] = res[i];
...
Table.prototype.load = function(where, order, done, doneOk, tx, lock, pgo) {
	...
	if(typeof(done) !== "function")
		throw new Error("Pgo.load: done must be a function");

	if(tx && tx.closed)
		return process.nextTick(done.bind(null, { pgo: {
			code: 1030,
			doing: errors[1030],
		...
		client.query(query, values, (err, res) => {
			...
			for(var i in rows) {
				...
				if(row.tableoid === this.oid) {
					...
					for(var l in row)
						rec[l] = Record.prototype.clone.call(row[l]);
					...
					reload[row.tableoid][
						row[this.pk]] = ret.length;
					ret.push(null);
				}
			}

			if(Object.keys(reload).length)
				return doReload(pgo, reload, ret, done, doneOk, Object.keys(reload), 0, tx);
...
Table.prototype.doSave = function(done, doneOk, record, client, donePG, query, values) {
	...
	client.query(query, values, (err, res) => {
		...
		for(var i in row)
			record[i] = Record.prototype.clone.call(row[i]);
...
Table.prototype.doUpdate = function(done, doneOk, record, values) {
	...
	if(! place)
		return process.nextTick(doneOk.bind(null, false));
...
Table.prototype.save = function(done, doneOk, record) {
	...
	if(this.pk in record.__obj)
		query = this.doUpdate(done, doneOk, record, values);
	else
	...
	if(record.__tx)
		return this.doSave(done, doneOk, record, record.__tx.client, empty, query, values);
