...
Pgo.prototype.TIMESTAMP = function(options) {
	if(! options)
		return { type: "timestamptz", precision: 6 };
	...
	return { type: "timestamptz", precision: precision, insertNow: options.insertNow };
};
...
Pgo.prototype.model = function(name, fields, options) {
	...
	for(var fld in fields) {
		...
		case "timestamptz":
			if(fldef.defaultValue && fldef.insertNow)
				throw new Error("Pgo.mopdel: only one of defaultValue or insertNow can be defined for '" + name + "." + fld + "'");

			if(fldef.defaultValue) {
				if("object" == typeof fldef.defaultValue && "toISOString" in fldef.defaultValue && "function" == typeof fldef.defaultValue.toISOString)
			...
			if(fldef.insertNow) {
				fldef.defaultValue = "CURRENT_TIMESTAMP";
				fldef.notNull = true;
			}
			break;
