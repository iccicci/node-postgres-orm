...
Pgo.prototype.TIMESTAMP = function(precision) {
	...
	return { type: "timestamptz", precision: precision };
};
	...
	pg.connect(this.database, function(err, client, done) {
		...
		client.query("BEGIN", {}, function(err) {
			...
			var createAccessors = function(record, table) {
				tx.load[record] = function(where, order, callback) { table.load(where, order, callback, tx); };
				tx.lock[record] = function(where, order, callback) { table.load(where, order, callback, tx, true); };
			};
