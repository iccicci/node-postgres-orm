...
Pgo.prototype.clone = function(log) {
	...
	var createLoad = function(name) {
		return function(where, order, callback, _pgo) {
			ret.tables[name + "s"].load(where, order, callback, null, null, _pgo || ret);
		};
	};
...
Pgo.prototype.__modelChecks = function(name, fields, options) {
	if(! this.schema)
		throw new Error(pgop + "Can't call Pgo.table after Pgo.connect");

	if(name in this.schema.thash)
	...
	if(options.attributes && typeof(options.attributes) != "object")
		throw new Error("Pgo.model: options.attributes must be an Object");

	this.schema.thash[name] = this.schema.tables.length;

	if(options.parent && this.schema.thash[options.parent] === undefined)
		throw new Error(pgop + "Parent model '" + options.parent + "' for model '" + name + "' doesn't exists");

	if(options.primaryKey && ! fields[options.primaryKey])
		throw new Error(pgop + "Undefined field: '" + name + "." + options.primaryKey + "' specified as primary key");

	return options;
};

Pgo.prototype.__modelFieldType = function(name, fields, options, fld) {
	var fldef;
...
Pgo.prototype.__modelFieldFK = function(name, fields, options, fld, fldef) {
	var i;
	var ref;
...
Pgo.prototype.__modelFieldDefault = function(name, fields, options, fld, fldef) {
	switch(fldef.type) {
	case "int2":
	case "int4":
	case "int8":
...
Pgo.prototype.__modelField = function(name, fields, options, fld, constraints, farray) {
	...
	if(fldef.fkTable)
		this.__modelFieldFK(name, fields, options, fld, fldef);
...
Pgo.prototype.model = function(name, fields, options) {
	...
	if(options.parent)
		options.pk = schema.tables[schema.thash[options.parent]].pk;
	else
		constraints.push({
	...
	for(var fld in fields)
		this.__modelField(name, fields, options, fld, constraints, farray);
