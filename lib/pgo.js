...
Pgo.prototype.begin = function(done, doneOk) {
	...
	if(typeof(done) !== "function")
		throw new Error("Pgo.begin: done must be a function");
	...
	pg.connect(this.database, doneOk.dd((client, donePG) => {
		...
		client.query("BEGIN", [], doneOk.dd(() => {
			...
			var createAccessors = (record, table) => {
				tx.load[record] = (where, order, done, doneOk) => { table.load(where, order, done, doneOk, tx, null, this); };
				tx.lock[record] = (where, order, done, doneOK) => { table.load(where, order, done, doneOk, tx, true, this); };
			};
