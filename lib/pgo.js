...
Pgo.prototype.TIMESTAMP = function(precision) {
  ...
  if(parseInt(precision, 10) !== precision || precision < 0 || precision > 6) throw new Error("Pgo.TIMESTAMP: precision must be an integer between 0 and 6");
...
Pgo.prototype.VARCHAR = function(len) {
  if(! len) return "varchar";

  if(parseInt(len, 10) !== len || len < 0) throw new Error("Pgo.VARCHAR: len must be a positive integer");
...
Pgo.prototype.begin = function(done, doneOk) {
  ...
  if(typeof done !== "function") throw new Error("Pgo.begin: done must be a function");

  pg.connect(
    this.database,
    doneOk.dd((client, donePG) => {
      doneOk = dd(function(err) {
        donePG();
        done(err);
      }, doneOk);
      ...
      client.query(
        ...
        doneOk.dd(() => {
          ...
          var createAccessors = (record, table) => {
            tx.load[record] = (where, order, done, doneOk) => {
              table.load(where, order, done, doneOk, tx, null, this);
            };
            tx.lock[record] = (where, order, done, doneOK) => {
              table.load(where, order, done, doneOk, tx, true, this);
            };
          };
          ...
          for(var record in this.models) createAccessors(record, this.tables[record]);
...
Pgo.prototype.clone = function(log) {
  ...
  var createLoad = function(name) {
    return function(where, order, done, doneOk, _pgo) {
      ret.tables[name].load(where, order, done, doneOk, null, null, _pgo || ret);
    };
  };
  ...
  for(var i in this.load) ret.load[i] = createLoad(i);

  for(i in this.models) ret.models[i] = createModel(i, this.models[i]);
...
Pgo.prototype.error = function(err, code, target, field) {
  ...
  if(target) err.pgo.target = target;

  if(field) err.pgo.field = field;
...
for(var i in model) Pgo.prototype[i] = model[i];
